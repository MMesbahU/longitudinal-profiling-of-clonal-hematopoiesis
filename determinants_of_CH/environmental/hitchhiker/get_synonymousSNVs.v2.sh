...
# zcat /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf/baseline/hiseq_baseline.nrow_1_500.mpos10_45.dp20_400.vaf35.tsv.gz | head -1 | gzip -c > /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf/baseline/all_HiSeq_baseline.mpos10_45.dp20_400.vaf35.tsv.gz
#cfor files in $(ls -l /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf/baseline/hiseq_baseline.nrow_*.mpos10_45.dp20_400.vaf35.tsv.gz | awk '{print $NF}'); do zcat ${files} | awk 'NR>1{print $0}' | gzip -c >> /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf/baseline/all_HiSeq_baseline.mpos10_45.dp20_400.vaf35.tsv.gz; done & 
...
# for i in `seq 1 500 $(wc -l /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf/hiseq_vcf_file.list | awk '{print $1}')`; do qsub -R y -wd /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf/tmpdir -N var_per_sam_${i}_$(( ${i} + 499 )) -l h_rt=20:00:00 -l h_vmem=20G -pe smp 1 -binding linear:1 /medpop/esp2/mesbah/tools/longitudinal-profiling-of-clonal-hematopoiesis/detect_CH/hitchhiker/get_synonymousSNVs.v2.sh /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf/hiseq_vcf_file.list ${i} $(( ${i} + 499 )) /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf/baseline mpos10_45.dp20_400.vaf35; done
