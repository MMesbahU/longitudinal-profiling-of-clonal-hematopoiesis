...
# qsub -N hiseq_vcf -R y -wd /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf -l h_rt=30:00:00 -l h_vmem=20G -pe smp 1 -binding linear:1 /medpop/esp2/mesbah/tools/longitudinal-profiling-of-clonal-hematopoiesis/detect_CH/hitchhiker/1.baseline_vcf.sh /medpop/esp2/mesbah/datasets/CHIP/ARIC/hiseq_vcf/annot
...
## bcftools view -H -i 'FILTER=="PASS" && MBQ[0]>=30 && MBQ[1]>=30 && MMQ[0]>=60 && MMQ[1]>=60 && FMT/DP>=20 && FMT/AD[0:0]>=3 && FMT/AD[0:1]>=3 && FMT/AF>=0.02 && FMT/AF<=0.3 && FMT/F1R2[0:0]>1 && FMT/F1R2[0:1]>1 && FMT/F2R1[0:0]>1 && FMT/F2R1[0:1]>1' /medpop/esp2/mesbah/projects/ch_progression/aric/pacer/test/annot.A08377-filtered.hg38_multianno.vcf.gz
##
gsutil -m cp gs://muddin/ARIC_realigned_hg38/annovar_vcf/*.vcf.gz ${outDir}/
##
