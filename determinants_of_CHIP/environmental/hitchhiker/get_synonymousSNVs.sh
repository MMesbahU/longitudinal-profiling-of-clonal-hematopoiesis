...
## after finised: to combine all files: zcat /datasets/CHIP/ARIC/hiseq_vcf/baseline/hiseq_baseline.nrow_1501_2000.tsv.gz | gzip -c > /datasets/CHIP/ARIC/hiseq_vcf/baseline/all_HiSeq_baseline.tsv.gz; for files in $(ls -lh /datasets/CHIP/ARIC/hiseq_vcf/baseline/hiseq_baseline.nrow_*.tsv.gz | awk '{print $NF}'); do zcat ${files} | awk 'NR>1{print $0}' | gzip -c >> /datasets/CHIP/ARIC/hiseq_vcf/baseline/all_HiSeq_baseline.tsv.gz; done & 
